...
const registro = async (req, res) => {
  const alterTableQuery = `
    ...
    ADD COLUMN ObjProteinas DECIMAL(10, 2),
    ADD COLUMN ObjCalorias DECIMAL(10, 2);
  `;

  db.query(alterTableQuery, (err, result) => {
    if (err) {
      console.error("Error al agregar columnas a la base de datos:", err);
    } else {
      console.log("Columnas agregadas exitosamente:", result);
    }
  });
